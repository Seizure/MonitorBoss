...
def test_caps_raw(json_flag, test_conf_file, capsys):
    expected = output.caps_raw_output([(m_data_0_foo, impl.get_vcp_capabilities(0))], json_flag) + "\n"
...
def test_caps_full(json_flag, test_conf_file, test_cfg, capsys):
    ...
    expected = output.caps_parsed_output([(m_data_0_foo, caps)], json_flag) + "\n"
...
def test_get_feature(json_flag, test_conf_file, test_cfg, capsys):
    ...
    expected = output.get_feature_output(info.feature_data(lum, test_cfg), [(m_data_0_foo, vdata, 80)], json_flag) + "\n"
...
def test_set_feature(json_flag, value, test_conf_file, test_cfg, capsys):
    ...
    expected = output.set_feature_output(info.feature_data(lum, test_cfg), [(m_data_0_foo, vdata)], json_flag) + "\n"
...
def test_tog_feature(json_flag, test_conf_file, test_cfg, capsys):
    ...
    expected = output.tog_feature_output(info.feature_data(lum, test_cfg), [(m_data_0_foo, vdata1, vdata2)], json_flag) + "\n"
