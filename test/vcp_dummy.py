...
class DummyVCP(VCP):
    ...
    def _set_vcp_feature(self, code: VCPCommand, value: int):
        ...
        if supported_codes[code] is None:
            if value > max_values[code]:
                raise ValueError(f"value of {value} exceeds code maximum of {max_values[code]} for {VCPCommand.name}")
            else:
    ...
    def _get_vcp_feature(self, code: VCPCommand) -> (int, int):
        ...
        if code not in supported_codes:
            # TODO: what should happen when an unsupported VCP command is sent?
            pass
        if not VCPCommand.discreet:
            maxv = max_values[code]
        else:
            maxv = 0
